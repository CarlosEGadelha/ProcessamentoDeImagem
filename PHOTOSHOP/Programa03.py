...
def salvar_thumbnail(filename, filelink, output):
    if  filelink != "":
        image = Image.open(request.urlopen(filelink))
    else:
...
def salvar_qualidade_reduzida(filename, filelink, output):
    if  filelink != "":
        image = Image.open(request.urlopen(filelink))
    else:
        if os.path.exists(filename):
    ...
    new_image = image
    new_image.save(output, quality = 0.1)

def salvar(filename, filelink, formato, new_filename):
    if  filelink != "":
        image = Image.open(request.urlopen(filelink))
    else:
        if os.path.exists(filename):
...
def salvar_branco_preto(filename, filelink, output):
    if  filelink != "":
        image = Image.open(request.urlopen(filelink))
    else:
        if os.path.exists(filename):
...
def converte_sepia(filename, filelink, output):
    branco = (255, 240, 192)
...
def diminuir_cor(filename, filelink, output): 
    if  filelink != "":
        image = Image.open(request.urlopen(filelink))
    else:
        if os.path.exists(filename):
    ...
    image.convert("L", palette=Image.Palette.ADAPTIVE, colors = 10)
    new_image = image
    new_image.save(output)         
...
def exif():
    ...
    while True:
        ...
        try:
            ...
            messagebox.showinfo("ERRO", "ERRO AO TENTAR PEGAR OS DADOS DA IMAGEM")

def mirror(filename, filelink, output):
    if  filelink != "":
        image = Image.open(request.urlopen(filelink))
    else:
        if os.path.exists(filename):
...
def rotate(filename, filelink,  degrees_to_rotate, output_image_path):
    if  filelink != "":
        image = Image.open(request.urlopen(filelink))
    else:
        if os.path.exists(filename):
...
def resize(filename, filelink, output_image_path):
    if  filelink != "":
        image = Image.open(request.urlopen(filelink))
    else:
        if os.path.exists(filename):
...
def brilho(filename, filelink, fator, output_filename):
    if  filelink != "":
        image = Image.open(request.urlopen(filelink))
    else:
        if os.path.exists(filename):
    ...
    image.thumbnail((500,500))
    imagem = image.convert('RGB')
    enhancer = ImageEnhance.Brightness(imagem)
    new_image = enhancer.enhance(fator)
    new_image.save(output_filename)

def contraste(filename, filelink, fator, output_filename):
    if  filelink != "":
        image = Image.open(request.urlopen(filelink))
    else:
        if os.path.exists(filename):
...
def cores(filename, filelink, fator, output_filename):
    if  filelink != "":
        image = Image.open(request.urlopen(filelink))
    else:
        if os.path.exists(filename):
...
def nitidez(filename, filelink, fator, output_filename):
    if  filelink != "":
        image = Image.open(request.urlopen(filelink))
    else:
        if os.path.exists(filename):
...
def aplica_efeito(values, tmp_file, window):
    ...
    filename = values["-FILE-"]
    filelink = values["-LINK-"]
    factor = values["-FATOR-"]
    
    # if filename:
    if efeito_selecionado == "Normal":
        efeitos[efeito_selecionado](filename, filelink, tmp_file)
    else:
        efeitos[efeito_selecionado](filename, filelink, factor, tmp_file)
...
def main():
    ...
    layout = [
        ...
        [
            sg.Text("Efeitos"),
            sg.Combo(effect_names, key="-EFEITOS-", enable_events=True, readonly=True),
    ...
    while True:
        ...
        if event == "Thumbnail":
            salvar_thumbnail(filename,filelink, tmp_file) 
            atualizar_imagem(tmp_file, window)

        if event == "Qualidade baixa":
            salvar_qualidade_reduzida(filename, filelink, tmp_file)
            atualizar_imagem(tmp_file, window)

        if event == "Salvar":
            # print("NOVO FILENAME: ",new_filename)
            salvar(filename, filelink, formato, new_filename)

        if event == "Preto e branco":
            salvar_branco_preto(filename, filelink, tmp_file)
            atualizar_imagem(tmp_file, window)    

        if event == "Sepia":
            converte_sepia(filename, filelink, tmp_file)
            atualizar_imagem(tmp_file, window)

        if event == "Diminuir cores":
            diminuir_cor(filename, filelink, tmp_file)
            atualizar_imagem(tmp_file, window)
        ...
        if event == "ESPELHADO":
            mirror(filename, filelink, tmp_file)
            atualizar_imagem(tmp_file, window)
        ...
        if event == "DIREITA":
            rotate(filename, filelink,90, tmp_file)
            atualizar_imagem(tmp_file, window)
        ...
        if event == "TAMANHO":
            resize(filename, filelink, tmp_file)
            atualizar_imagem(tmp_file, window)
